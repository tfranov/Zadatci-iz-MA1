...
\end{exercise}
\begin{proof}[Rješenje]
Promotrimo izraz \(D=\left(\sum\limits_{k=1}^n{\,a_kb_k}\right)^2- \left(\sum\limits_{k=1}^n{\,a_k^2}\right)\cdot\left(\sum\limits_{k=1}^n{\,b_k^2}\right)\). Trebamo pokazati da je \(D\leq 0\). No uočimo da je \(D\) diskriminanta kvadratne funkcije \(f : \mathbb{R}\to \mathbb{R}\),
\[f(x)=\dfrac{1}{2}\cdot\left(\sum\limits_{k=1}^n{\,a_k^2}\right)x^2+\left(\sum\limits_{k=1}^n{\,a_kb_k}\right)x+\dfrac{1}{2}\left(\sum\limits_{k=1}^n{\,b_k^2}\right).\]
...
\[f(x)=\dfrac{1}{2}\,\sum\limits_{k=1}^n{\,a_k^2x^2+2a_kb_kx+b_k^2}=\dfrac{1}{2}\,\sum\limits_{k=1}^n{\,(a_kx+b_k)^2}\geq 0,\]
što smo i htjeli pokazati. Nadalje, znamo da je \(D=0\) ako i samo ako \(f\) ima jedinstvenu nultočku \(x_0\). Tada je \[\sum\limits_{k=1}^n{\,(a_kx_0+b_k)^2}=0,\] što vrijedi ako i samo ako je \(a_kx_0+b_k=0\) za sve \(k=1, \dots, n\).\footnote{Općenito, za sve \(x_1, \dots, x_n\in \mathbb{R}\) vrijedi \(x_1^2+\dots+x_n^2=0\) ako i samo ako vrijedi \(x_1=\dots=x_n=0\). Dokažite to!} No to je ekvivalentno tvrdnji \(b_k=(-x_0)a_k\) za sve \(k=1, \dots, n\), što smo i htjeli dokazati.
\end{proof}
...
\item[c)] Dokažite tvrdnju iz b) dijela napomene \ref{den}. 
\end{itemize}
\end{exercise}
\begin{proof}[Rješenje]
