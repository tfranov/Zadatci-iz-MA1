...
\begin{itemize}
\item[a)] $f_1 : \mathbb{R}\to \mathbb{R}$, $f(x)=\sin{x}$,
\item[b)] $f_2 : \langle 0, \infty\rangle\to \mathbb{R}$, $f(x)=\ln{x}$,
\item[c)] $f_3 : [0,\infty\rangle\to \mathbb{R}$, $f(x)=\sqrt{x}$. 
\end{itemize}
\end{exercise}
\begin{proof}[Rješenje]
...
$$\left|\ln\left(\dfrac{x_\delta}{y_\delta}\right)\right|=\left|\ln\left(\dfrac{\delta}{\frac{\delta}{3}}\right)\right|=\ln{3}\geq 1.$$

c) Tvrdimo da je $f_3$ uniformno neprekidna. Uočimo da je u dokazu neprekidnosti ove funkcije u a) dijelu zadatka \ref{cont4} naš izbor broja $\delta$ ovisio o $c$, pa ćemo umjesto "imitiranja" dokaza tvrdnje za neprekidnost postupiti drugačije -- uočimo da ako promotrimo $f_3|_{[1, \infty\rangle}$, onda ponavljanjem argumenta iz zadatka \ref{cont4} dobivamo da za proizvoljan $\epsilon>0$ i $\delta=\epsilon$ dobivamo da za sve $x, y\geq 1$ takve da je $|x-y|<\delta$ dobivamo
$$\left|\sqrt{x}-\sqrt{y}\right|<\dfrac{|x-y|}{\sqrt{y}}\leq |x-y|<\delta=\epsilon,$$
što znači da je $f_3|_{[1, \infty\rangle}$ uniformno neprekidna na $[1, \infty\rangle$. 

S druge strane, uniformna neprekidnost funkcije $f_3|_{[0, 1]}$ na svojoj domeni slijedi iz činjenice da je $f_3|_{[0, 1]}$ neprekidna na svojoj domeni (jer je $f_3$ neprekidna na svojoj domeni), pa je ona i uniformno neprekidna na svojoj domeni.

Sada će ideja sljedeća -- ako uzmemo "manju" od dvije "delte" koje dobivamo da su dobre za ove dvije funkcije, dobivamo da ćemo za bliske $x, y$ koji su oboje ili u $[0, 1]$ ili u $[1, \infty\rangle$ dobiti tvrdnju iz definicije uniformne neprekidnosti. Naravno, može se dogoditi da je npr.
\end{proof}
\newpage
