...
\end{remark}
Pojam Cauchyjevih nizova je posebno bitan u drugim scenarijima (npr. nizovi u $\mathbb{Q}$) gdje Cauchyjevost niza ne mora biti isto što i njegova konvergencija. No, ovaj pojam je koristan i za naše svrhe jer može poslužiti u rješavanju raznih problema koji možda ne bi bili toliko lako rješivi inače.
\begin{exercise}
Dokažite po definiciji da je niz $(a_n)$ zadan s $a_n=\dfrac{1}{n^2}$ Cauchyjev.
\end{exercise}
\begin{proof*}
Neka je $\epsilon>0$. Uzmimo $n_0\in \mathbb{N}$ takav da je $n_0\epsilon>2$. Tada za svaki $l\geq n_0$ vrijedi
...
\[
\pushQED{\qed}
\dfrac{1}{n^2}-\dfrac{1}{m^2}=\dfrac{m^2-n^2}{n^2m^2}=\dfrac{m^2-n^2}{n^2m^2}=\dfrac{(m-n)(m+n)}{n^2m^2}\leq \dfrac{m\cdot 2m}{n^2m^2}=\dfrac{2}{n^2}<\epsilon.\qedhere
\popQED
\]
\end{proof*}
NASTAVITI!!!
...
\end{align*}
Dokažimo sada prvu nejednakost, i to koristeći već dokazanu drugu nejednakost. Uočimo da je
\begin{align*}
\limsup{(a_n+b_n)}-\liminf{a_n}&=\limsup{(a_n+b_n)}+\limsup{-a_n}\geq \limsup{(a_n+b_n-a_n)}\\
&=\limsup{b_n},
\end{align*}
...
\end{remark}
DODATI CAUCHYJEVE NIZOVE RANIJE!! ZAMIJENI LVERT SA LEFT LVERT!!
\newpage
