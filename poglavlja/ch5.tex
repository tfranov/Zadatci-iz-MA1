...
\begin{definition}
Neka je \((a_n)\) niz realnih brojeva. Kažemo da je \(L\in \mathbb{R}\) \textbf{limes} niza \((a_n)\) ako
\begin{gather*}
...
\[\left\lvert\dfrac{n}{n+1}-1\right\rvert=\dfrac{1}{n+1}<\epsilon.\] Prema Arhimedovu aksiomu znamo da postoji \(n_0\in \mathbb{N}\) takav da je \(n_0\epsilon>1\). Dokažimo da tvrdnja vrijedi i za sve \(n\geq n_0\). Zaista, ako je \(n\geq n_0\), onda vrijedi i \(n+1\geq n_0+1\), odnosno \[(n+1)\epsilon\geq(n_0+1)\epsilon>n_0\epsilon>1.\] 
...
\[\left\lvert \dfrac{n}{n^2+1}-0\right\rvert=\dfrac{n}{n^2+1}<\epsilon.\]
...
\end{exercise}
\begin{proof}[Rješenje]
Znamo da za sve \(\epsilon>0\) postoji \(n_0\in \mathbb{N}\) takav da za sve \(n\geq n_0\) vrijedi \(a_n>\dfrac{1}{\epsilon}\), odakle slijedi \(\dfrac{1}{a_n}<\epsilon\), jer je \(a_n>0\). No ujedno vrijedi i \(\left\lvert \dfrac{1}{a_n}\right\rvert<\epsilon\), ponovno zbog \(a_n>0\).
\end{proof}
\begin{exercise}{Zadatak}
\label{liminftylemma}
...
\end{proof}
\begin{exercise}{Zadatak}
\label{Thisisobviousriiiiiiight}
...
Ideja će biti pokušati pronaći "što jednostavniji" izraz veći od \(\sqrt[n]{a}-1\), ali i dalje takav da možemo naći \(n_0\) takav da je za sve \(n\geq n_0\) manji od \(\epsilon\). Eksperimentiranjem s različitim nizovima možemo naslutiti da možemo uzeti \(\dfrac{a}{n}\), za kojeg se lako vidi da konvergira u \(0\). Dakle, zapravo se želimo uvjeriti da vrijedi \(\sqrt[n]{a}-1<\dfrac{a}{n}\), odnosno \(n\left(\sqrt[n]{a}-1\right)<a\) za sve \(n\in \mathbb{N}\). Kako treba doći od \(a=\left(\sqrt[n]{a}\right)^n\) do \(\sqrt[n]{a}\), ovdje je prirodna ideja primijeniti binomni teorem na \(a\).

Uzmimo \(n_0\in \mathbb{N}\) takav da je \(n_0\epsilon>a\), te neka je \(n\geq n_0\) proizvoljan. Ako je \(a>1\), onda je i \(\sqrt[n]{a}>1\). Sada slično kao u rješenju zadatka \ref{liman} imamo
...
\item Niz \(\left(|a_n|\right)\) je konvergentan i vrijedi \(\lim\limits_{n\to \infty}{|a_n|}=\left\lvert \lim\limits_{n\to \infty}{a_n}\right\rvert\).
\end{itemize}
\end{remark}
...
\end{itemize}
\end{remark}
\begin{exercise}{Zadatak} Odredite sljedeće limese.
\begin{itemize}
...
\end{remark}
\begin{remark}[O opisu supremuma i infimuma skupa koristeći konvergentne nizove]
\label{supinflimitpoint}
Neka je $A\subseteq \mathbb{R}$ neprazan i ograničen odozgo (odozdo). Tada postoji niz $(a_n)$ u $A$ takav da je $\sup{A}=\lim\limits_{n\to \infty}{a_n}$ $\left(\inf{A}=\lim\limits_{n\to \infty}{a_n}\right)$.
\end{remark}
\begin{exercise}{Zadatak} Odredite sljedeće limese, ako postoje.
\begin{itemize}
...
dakle \(n_0\epsilon>1\). No i za sve \(m\in \mathbb{N}\), \(m\geq 5\) vrijedi \(m<\dfrac{2^m}{m},\) što se lako pokazuje indukcijom. Stoga za sve prirodne \(n\geq n_0\) vrijedi \(\dfrac{2^{n}}{n}\epsilon>1\), odakle imamo \[\dfrac{n}{2^n}=\left\lvert \dfrac{n}{2^n}\right\rvert<\epsilon,\]
...
\end{proof}
\begin{exercise}{Zadatak}
\label{generalizedsubsequences}
Neka je $(a_n)$ niz. Kažemo da je niz $(b_n)$ \textit{generalizirani podniz} niza $(b_n)$ ako postoji rastući niz $(p_n)$ prirodnih brojeva takav da za svaki $n\in \mathbb{N}$ postoji $m\in \mathbb{N}$, $m>n$ takav da je $p_m>p_n$, sa svojstvom da za sve $n\in \mathbb{N}$ vrijedi $b_n=a_{p_n}$. Dokažite da je generalizirani podniz konvergentnog niza konvergentan s istim limesom kao i početni niz.
\end{exercise}
\begin{remark}
Intuitivno, generalizirani podnizovi se ponašaju analogno kao i podnizovi do na to što dopuštamo ponavljanje bilo kojeg elementa konačno mnogo puta.
\end{remark}
\begin{proof}[Rješenje]
Modificirat ćemo dokaz napomene \ref{onsubsequences} tako da uvjet $p_n\geq n$ koji se koristio u toj napomeni zamijenimo s analognim uvjetom koji će ovdje vrijediti. Uzmimo proizvoljan $\epsilon>0$, te proizvoljan $n\in \mathbb{N}$ takav da je $n\geq n_0$. Tvrdimo da postoji $m_0\in \mathbb{N}$ takav da je $p_{m_0}\geq n$. Ako je $p_1\geq n$, nemamo što dokazivati. Inače, postoji $b\in \mathbb{N}$ takav da je $p_{b}>p_1$. Ako je $p_{b}\geq n$, tvrdnja je dokazana, a inače u konačno mnogo koraka dolazimo do traženog $m_0$. Neka je sada $m\geq m_0$ proizvoljan. Specijalno je $p_m\geq p_{m_0}\geq n$, pa je $\lvert a_{p_m}-a\rvert<\epsilon$, što je i trebalo pokazati.
\end{proof}
\begin{exercise}{Zadatak}
\label{setoflimits}
Neka je \((a_n)\) konvergentan niz takav da je \(a_n\neq 0\) za sve \(n\in \mathbb{N}\). Konvergira li općenito niz \(\left(\dfrac{a_{n+1}}{a_n}\right)\)? Za one $(a_n)$ za koje on konvergira, što sve može biti njegov limes?
\end{exercise}
\begin{proof}[Rješenje]
...
Dokažimo b). Ako je npr. \(a\in \langle 1, \infty\rangle\), onda postoji \(m_0\in \mathbb{N}\) takav da za sve prirodne \(m\geq m_0\) vrijedi \[\left\lvert \dfrac{a_{m+1}}{a_m}-a\right\rvert<\dfrac{a-1}{2},\]
odakle specijalno imamo
\[\dfrac{a_{m+1}}{a_m}-a>-\dfrac{a-1}{2},\;\text{ odnosno }\; \dfrac{a_{m+1}}{a_m}>1.\]
Kako je \(\dfrac{a_{m+1}}{a_m}=\left\lvert \dfrac{a_{m+1}}{a_m}\right\rvert\), slijedi \(|a_{m+1}|>|a_m|\) za sve \(m\geq m_0\). Odavde lako slijedi da je \(|a_{n}|>|a_m|\) za sve \(n>m\geq n_0\). Tvrdnja se analogno pokazuje za \(a\in \langle -\infty, -1\rangle\).
...
\[0\leq \left\lvert -\dfrac{1}{n}\sin{\dfrac{n\pi}{2}}\right\rvert=\dfrac{1}{n}\left\lvert \sin{\dfrac{n\pi}{2}}\right\rvert\leq \dfrac{1}{n},\]
...
\end{proof}
Za zadatak \ref{38} bit će nam korisna sljedeća pomoćna tvrdnja.
\begin{exercise}{Zadatak}
...
\end{proof}
\section{Limesi složenijih nizova. Cauchyjevi nizovi}
...
\begin{gather}
\label{35}
\dfrac{\epsilon}{2}>\left\lvert a_{n-1}+2a_n-a\right\rvert=\left\lvert 2\left(a_n-\dfrac{a}{3}\right)+a_{n-1}-\dfrac{a}{3}\right\rvert,
\end{gather}
pa zbog činjenice da za sve \(x, y\in \mathbb{R}\) vrijedi \(||x|-|y||\leq |x-y|\), odakle slijedi \(|x|-|y|=|x|-|-y|\leq |x+y|\), vrijedi
\[\left\lvert 2\left(a_n-\dfrac{a}{3}\right)+a_{n-1}-\dfrac{a}{3}\right\rvert\geq 2\left\lvert a_n-\dfrac{a}{3}\right\rvert-\left\lvert a_{n-1}-\dfrac{a}{3}\right\rvert,\]
odnosno
\[\left\lvert a_n-\dfrac{a}{3}\right\rvert<\dfrac{\epsilon}{4}+\dfrac{1}{2}\left\lvert a_{n-1}-\dfrac{a}{3}\right\rvert.\]
Tada za sve \(m\in \mathbb{N}\) vrijedi
\begin{align*}
\left\lvert a_{n+m}-\dfrac{a}{3}\right\rvert&<\dfrac{\epsilon}{4}+\dfrac{1}{2}\left\lvert a_{n+m-1}-\dfrac{a}{3}\right\rvert<\dfrac{\epsilon}{4}+\dfrac{1}{2}\left(\dfrac{\epsilon}{4}+\dfrac{1}{2}\left\lvert a_{n+m-1}-\dfrac{a}{3}\right\rvert\right)\\
&=\dfrac{\epsilon}{4}+\dfrac{1}{2}\cdot\dfrac{\epsilon}{4}+\dfrac{1}{4}\left\lvert a_{n+m-1}-\dfrac{a}{3}\right\rvert=\dots=\dfrac{\epsilon}{4}\sum_{k=0}^m{\dfrac{1}{2^m}}+\dfrac{1}{2^{m+1}}\left\lvert a_{n-1}-\dfrac{a}{3}\right\rvert.
\end{align*}
Uočimo da je \[\sum_{k=0}^m{\dfrac{1}{2^m}}=2\left(1-\dfrac{1}{2^n}\right)<2.\]
Zato je
\begin{gather}
\label{36}
\left\lvert a_{n+m}-\dfrac{a}{3}\right\rvert<\dfrac{\epsilon}{4}\sum_{k=0}^m{\dfrac{1}{2^m}}+\dfrac{1}{2^{m+1}}\left\lvert a_{n-1}-\dfrac{a}{3}\right\rvert<\dfrac{\epsilon}{2}+\dfrac{1}{2^{m+1}}\left\lvert a_{n-1}-\dfrac{a}{3}\right\rvert.
\end{gather}
Prema Arhimedovu aksiomu, postoji \(m_0\in \mathbb{N}\) takav da je \(m_0\cdot\dfrac{\epsilon}{2}>\left\lvert a_{n-1}-\dfrac{a}{3}\right\rvert\). Tada za sve \(m\geq m_0\) vrijedi
\[\left\lvert a_{n-1}-\dfrac{a}{3}\right\rvert<m_0\cdot\dfrac{\epsilon}{2}<(m_0+1)\cdot\dfrac{\epsilon}{2}<2^{m_0+1}\cdot\dfrac{\epsilon}{2}<2^{m+1}\cdot\dfrac{\epsilon}{2},\]
odnosno
\begin{gather}
\label{37}
\dfrac{1}{2^{m+1}}\left\lvert a_{n-1}-\dfrac{a}{3}\right\rvert<\dfrac{\epsilon}{2}.
\end{gather}
Sada iz (\ref{35}), (\ref{36}) i (\ref{37}) slijedi da za sve \(n\geq n_0\) i \(m\geq m_0\) vrijedi
\[\left\lvert a_{n+m}-\dfrac{a}{3}\right\rvert<\epsilon,\;\text{ tj. }\; \left\lvert a_{p}-\dfrac{a}{3}\right\rvert<\epsilon,\; \forall p\in \mathbb{N}\;\text{ za kojeg je }\; p\geq n_0+m_0,\]
što smo i tvrdili.
\end{proof}
\begin{exercise}{Zadatak}[Cesàro-Stolzov teorem] Neka su \((a_n)\) i \((b_n)\) nizovi takvi da je \(b_n\neq 0\) za sve \(n\in \mathbb{N}\), te \((b_n)\) strogo rastući i neka je \(\lim\limits_{n\to \infty}{b_n}=\infty\). Ako niz \(\left(\dfrac{a_{n+1}-a_n}{b_{n+1}-b_n}\right)\) konvergira u \(L\in \mathbb{R}\), onda i niz \(\left(\dfrac{a_n}{b_n}\right)\) konvergira u \(L\).
\end{exercise}
\begin{proof}[Rješenje]
Po definiciji, za svaki \(\epsilon>0\) postoji \(n_0\in \mathbb{N}\) takav da za sve prirodne \(N\geq N_0\) vrijedi
\[\left\lvert \dfrac{a_{N+1}-a_N}{b_{N+1}-b_N}-L\right\rvert<\dfrac{\epsilon}{2},\]
...
\[\left\lvert \dfrac{a_{n+1}}{b_{n+1}}-L\right\rvert<\epsilon,\]
...
\end{exercise}
\begin{proof}[Rješenje]
Promotrimo nizove \((a_n)\), \((b_n)\), \(a_n=\sum\limits_{k=n}^{2n}{k^4}\), \(b_n=n^5\). Uočimo da je \(b_n\) rastući, te kako vrijedi \(b_n\geq n\), za sve \(n\in \mathbb{N}\), iz zadatka \ref{liminftylemma} slijedi da je \(\lim\limits_{n\to \infty}{b_n}=\infty\), te \(b_n\neq 0\). Prema Cesàro-Stolzovom teoremu imamo
\begin{align*}
\lim\limits_{n\to \infty}{\dfrac{a_n}{b_n}}&=\lim\limits_{n\to\infty}{\dfrac{a_{n+1}-a_n}{b_{n+1}-b_n}}
=\lim\limits_{n\to \infty}{\dfrac{\sum\limits_{k=n+1}^{2n+2}{k^4}-\sum\limits_{k=n}^{2n}{k^4}}{(n+1)^5-n^5}}=\lim\limits_{n\to \infty}{\dfrac{(2n+1)^4+(2n+2)^4-(n+1)^4}{(n+1)^5-n^5}}\\
&=\dfrac{31n^4+92n^3+114n^2+68n+16}{5n^4+10n^3+10n^2+5n+1}=\dfrac{31}{5}.\qedhere
\end{align*}
\end{proof}
\begin{definition}
Neka je $(a_n)$ niz realnih brojeva. Kažemo da je $(a_n)$ \textbf{Cauchyjev niz} ako za svaki $\epsilon>0$ postoji $n_0\in \mathbb{N}$ takav da za sve $n, m\geq n_0$ vrijedi $\lvert a_n-a_m\rvert<\epsilon$.
\end{definition}
\begin{remark}
Niz realnih brojeva je Cauchyjev ako i samo ako je konvergentan.
\end{remark}
Pojam Cauchyjevih nizova je posebno bitan u općenitijim scenarijima, gdje Cauchyjevost niza ne mora biti isto što i njegova konvergencija, no biti će koristan i za naše svrhe, pogotovo u zadatcima gdje želimo pokazati konvergenciju niza čiji limes nužno ne znamo, a znamo da su "daleki" članovi niza međusobno "po volji bliski" (što je upravo definicija Cauchyjevog niza).
\begin{exercise}{Zadatak} \textbf{}
\begin{itemize}
\item[a)] Dokažite po definiciji da je niz $(a_n)$ zadan s $a_n=\dfrac{1}{n^2}$ Cauchyjev.
\item[b)] Dokažite da niz $(a_n)$ zadan s $a_n=1+\dfrac{1}{2}+\dfrac{1}{3}+\dots+\dfrac{1}{n}$ divergira.
\end{itemize}
\end{exercise}
\begin{proof}
a) Neka je $\epsilon>0$. Uzmimo $n_0\in \mathbb{N}$ takav da je $n_0\epsilon>2$. Tada za svaki $l\geq n_0$ vrijedi
$$l^2\epsilon\geq l\epsilon\geq n_0\epsilon>2.$$
Neka su sada $m, n\in \mathbb{N}$ takvi da vrijedi $m, n\geq n_0$. Bez smanjenja općenitosti možemo uzeti $m\geq n$. Vrijedi
\[
\dfrac{1}{n^2}-\dfrac{1}{m^2}=\dfrac{m^2-n^2}{n^2m^2}=\dfrac{m^2-n^2}{n^2m^2}=\dfrac{(m-n)(m+n)}{n^2m^2}\leq \dfrac{m\cdot 2m}{n^2m^2}=\dfrac{2}{n^2}<\epsilon.
\]
b) Pokazat ćemo da $(a_n)$ nije Cauchyjev. Zaista, neka je $\epsilon=\dfrac{1}{2}$ i $n_0\in \mathbb{N}$. Tada vrijedi
$$\lvert a_{2n_0}-a_{n_0}\rvert=\dfrac{1}{n_0+1}+\dfrac{1}{n_0+2}+\dots+\dfrac{1}{2n_0}\geq \dfrac{n_0}{2n_0}=\dfrac{1}{2}.$$
Stoga vrijedi negacija definicije Cauchyjevog niza, što smo i tvrdili.
\end{proof}
\begin{exercise}{Zadatak}
Neka je $(a_n)$ niz takav da postoji $c>0$ takav da za sve $n\in \mathbb{N}\setminus\{1\}$ vrijedi
$$\lvert x_2-x_1\rvert+\lvert x_3-x_2\rvert+\dots+\lvert x_n-x_{n-1}\rvert<c.$$
Dokažite da niz $(a_n)$ konvergira.
\end{exercise}
\begin{proof}[Rješenje]
Iz uvjeta zadatka naslućujemo da je niz Cauchyjev ("dovoljno daleki članovi niza su međusobno po volji bliski"). Zaista, neka je $\epsilon>0$, te neka je $(y_n)$ niz zadan s
$$y_n=\lvert x_2-x_1\rvert+\lvert x_3-x_2\rvert+\dots+\lvert x_n-x_{n-1}\rvert.$$
Kako je $(y_n)$ rastući i odozgo ograničen, on je konvergentan, pa i Cauchyjev, pa postoji $n_0\in \mathbb{N}$ takav da za sve $m\geq n_0$ i $q\in \mathbb{N}$ vrijedi $\lvert y_{m+q}-y_{m}\rvert=y_{m+q}-y_{m}<\epsilon$. Neka je sada $n\geq n_0$ i $p\in \mathbb{N}$. vrijedi
\begin{align*}
\lvert x_{n+p}-x_n\rvert&=\lvert x_{n+1}-x_{n}+x_{n-2}-x_{n+1}+\dots+ x_{n+p}-x_{n+p-1}\rvert\\
&\leq \lvert x_{n+1}-x_{n}\rvert+\lvert x_{n-2}-x_{n+1}\rvert+\dots+ \lvert x_{n+p}-x_{n+p-1}\rvert=y_{n+p}-y_{n}<\epsilon. \qedhere
\end{align*}
\end{proof}
\begin{exercise}{Zadatak}
Neka je $(a_n)$ niz sa svojstvom da niz
$\left(2a_{n+1}+\sin{a_n}\right)$ konvergira. Dokažite da tada i $(a_n)$ konvergira.
\end{exercise}
\begin{proof}[Rješenje]
Neka je $\epsilon>0$. Kako je $\left(2a_{n+1}+\sin{a_n}\right)$ Cauchyjev niz, postoji $n_0\in \mathbb{N}$ takav da za sve $m\geq n_0$ i $q\in \mathbb{N}$ vrijedi
$$\lvert b_{m+q}-b_m\rvert=\lvert 2a_{m+q+1}+\sin{a_{m+q}}-2a_{m+1}-\sin{a_m}\rvert<\dfrac{\epsilon}{2}.$$
Dokažimo da je i $(a_n)$ Cauchyjev niz. Uočimo prvo da je $(a_n)$ ograničen. Zaista, neka je $M>0$ takav da je $\lvert 2a_{n+1}+\sin{a_n}\rvert\leq M$ svaki $n\in \mathbb{N}$. Tada za proizvoljan $n\in \mathbb{N}$ imamo
\begin{gather*}
M\geq 2a_{n+1}+\sin{a_n}\geq 2{a_{n+1}}-1\Rightarrow a_{n+1}\leq \dfrac{M+1}{2},\\
-M\leq 2a_{n+1}+\sin{a_n}\leq 2a_{n+1}+1\Rightarrow a_{n+1}\geq \dfrac{-M-1}{2},
\end{gather*}
pa za $M':=\max{\left\{\dfrac{M+1}{2}, \lvert a_{1}\rvert\right\}}$ imamo $|a_n|\leq M'$. Uzmimo $n_1\in \mathbb{N}$ takav da je
$$n_0+1\leq n_1\;\;\text{ i }\;\;\dfrac{2M}{2^{n_1-(n_0+1)}}<\epsilon.$$
Tada za sve $n\geq n_1$ i $p\in \mathbb{N}$ vrijedi
\begin{align*}
\lvert a_{n+p}-a_{n}\rvert&=\left\lvert a_{n+p}+\dfrac{1}{2}\sin{a_{n+p-1}}-a_{n}-\dfrac{1}{2}\sin{a_{n-1}}+\dfrac{1}{2}\sin{a_{n-1}-\dfrac{1}{2}\sin{a_{n+p-1}}}\right\rvert\\
&\leq \left\lvert a_{n+p}+\dfrac{1}{2}\sin{a_{n+p-1}}-a_{n}-\dfrac{1}{2}\sin{a_{n-1}}\right\rvert+\left\lvert\dfrac{1}{2}\sin{a_{n-1}-\dfrac{1}{2}\sin{a_{n+p-1}}}\right\rvert\\
&<\dfrac{\epsilon}{4}+\dfrac{1}{2}\left\lvert\sin{a_{n-1}-\sin{a_{n+p-1}}}\right\rvert.
\end{align*}
Uočimo da je
\begin{align*}
\left\lvert\sin{a_{n-1}-\sin{a_{n+p-1}}}\right\rvert&=2\left\lvert \dfrac{\sin{\left(a_{n-1}-a_{n+p-1}\right)}}{2}\right\rvert\left\lvert \dfrac{\cos{\left(a_{n-1}+a_{n+p-1}\right)}}{2}\right\rvert\\
&\leq 2\left\lvert \dfrac{\sin{\left(a_{n-1}-a_{n+p-1}\right)}}{2}\right\rvert\leq 2\cdot \dfrac{\left\lvert a_{n-1}-a_{n+p-1}\right\rvert}{2}=\left\lvert a_{n-1}-a_{n+p-1}\right\rvert.
\end{align*}
Stoga imamo
\begin{align*}
\dfrac{\epsilon}{4}+\dfrac{1}{2}\left\lvert\sin{a_{n-1}-\sin{a_{n+p-1}}}\right\rvert&\leq \dfrac{\epsilon}{4}+\dfrac{1}{2}\left\lvert a_{n-1}-a_{n+p-1}\right\rvert\leq \dfrac{\epsilon}{4}+\dfrac{\epsilon}{8}+\dfrac{1}{4}\left\lvert a_{n-2}-a_{n+p-2}\right\rvert\\
&\leq \dfrac{\epsilon}{4}+\dfrac{\epsilon}{8}+\dots+\dfrac{\epsilon}{2\cdot 2^{n-(n_0+1)}}+\dfrac{1}{2^{n-(n_0+1)}}\left\lvert a_{n_0+1}-a_{n_0+1+p}\right\rvert\\
&<\dfrac{\epsilon}{2}+\dfrac{1}{2^{n-(n_0+1)}}\left\lvert a_{n_0+1}-a_{n_0+1+p}\right\rvert\\
&\leq \dfrac{\epsilon}{2}+\dfrac{1}{2^{n-(n_0+1)}}\left(\left\lvert a_{n_0+1}\right\rvert+\left\lvert a_{n_0+1+p}\right\rvert\right)=\dfrac{\epsilon}{2}+\dfrac{2M}{2^{n-(n_0+1)}}\\
&\leq \dfrac{\epsilon}{2}+\dfrac{2M}{2^{n_1-(n_0+1)}}<\epsilon. \qedhere
\end{align*}
\end{proof}
...
Za \(\epsilon_2=\left\lvert \alpha-a_{p_1}\right\rvert\) postoji \(a_{p_2}\in \left\langle \alpha-\epsilon_2, \alpha+\epsilon_2\right\rangle\setminus \{\alpha\}\) i vrijedi \(a_{p_2}\neq a_{p_1}\). Naime, ako je \(\alpha>a_{p_1}\), onda je \[a_{p_2}>\alpha-\left\lvert \alpha-a_{p_1}\right\rvert=\alpha-(\alpha-a_{p_1})=a_{p_1}\]
...
\begin{remark}
\label{bwlemma}
Svaki niz realnih brojeva ima monoton podniz.
\end{remark}
...
\end{proof}
\begin{remark}
Vidjeli smo da je računanje limesa inferiora i superiora u zadatku \ref{limitpoints2} bio poprilično dugotrajan posao. Srećom, postoji rezultat koji znatno olakšava traženje limesa inferiora, i to sljedeći -- Neka su \((a_n)\) i \((b_n)\) ograničeni nizovi i neka je \((a_n)\) konvergentan. Tada su nizovi \((a_n+b_n)\) i \((a_nb_n)\) ograničeni i vrijedi
\begin{gather}
\label{limsupformulas}
\limsup(a_n+b_n)=\lim\limits_{n\to \infty}(a_n)+\limsup(b_n),\\
\liminf(a_n+b_n)=\lim\limits_{n\to \infty}(a_n)+\liminf(b_n),
\end{gather}
Ako je pritom $a_n, b_n\geq 0$ za sve $n\in \mathbb{N}$, onda vrijedi
\begin{gather}
\limsup(a_nb_n)=\lim\limits_{n\to \infty}(a_n)\limsup(b_n),\\
\liminf(a_nb_n)=\lim\limits_{n\to \infty}(a_n)\liminf(b_n).
\end{gather}
Dokaz formule (\ref{limsupformulas}) dan je kao rješenje zadatka \ref{limsupofsum}. Ostale formule ovdje navedene mogu se dokazati sličnim postupkom kao u rješenju tog zadatka.
\end{remark}
\begin{remark}
...
\end{proof}
\begin{exercise}{Zadatak}
\label{limsupisalimitpoint}
Neka je $(a_n)$ ograničen niz. Dokažite da je tada $\limsup{a_n}$ gomilište niza $(a_n)$ (isto vrijedi i za $\liminf{a_n}$).
\end{exercise}
\begin{proof}[Rješenje]
Označimo sa $S$ skup svih gomilišta niza $(a_n)$, te uzmimo $a:=\limsup{a_n}$. Pretpostavimo da $a\notin S$. Tada po napomeni \ref{supinflimitpoint} postoji niz $(b_n)$ u $S$ koji konvergira k $a$. Neka je sada $\epsilon>0$. Tada postoji $n_0\in \mathbb{N}$ takav da je $|b_{n_0}-a|<\epsilon$, tj. $b_{n_0}\in \langle a-\epsilon, a+\epsilon\rangle\setminus{\left\{a\right\}}$. Kako je $b_{n_0}$ gomilište niza $(a_n)$, za $r:=\min\left\{ b_{n_0}-(a-\epsilon), a-b_{n_0}\right\}$ postoji bar jedan član niza $(a_n)$ u skupu $\langle b_{n_0}-r, b_{n_0}+r\rangle\setminus{\left\{b_{n_0}\right\}}$. Sada uočimo da je
$$\langle b_{n_0}-r, b_{n_0}+r\rangle\setminus{\left\{b_{n_0}\right\}}\subseteq \langle a-\epsilon, a+\epsilon\rangle\setminus{\left\{a\right\}}.$$
Zaista, vrijedi $b_{n_0}<a$ po definiciji od $a$ i iz pretpostavke da $a\notin S$. Odavde slijedi da je $b_{n_0}+r\leq b_{n_0}+a-b_{n_0}=a$, te $b_{n_0}-r\geq b_{n_0}-b_{n_0}+a-\epsilon=a-\epsilon$. Dakle, svaki element skupa $\langle b_{n_0}-r, b_{n_0}+r\rangle\setminus{\left\{b_{n_0}\right\}}$ je element skupa $\langle a-\epsilon, a\rangle$, što dokazuje tvrdnju.
\end{proof}
\begin{exercise}{Zadatak} \textbf{}
\label{limsupofsum}
\begin{itemize}
\item[a)] Dokažite da za ograničene nizove $(a_n)$ i $(b_n)$ vrijedi
$$\liminf{a_n}+\limsup{b_n}\leq \limsup{(a_n+b_n)}\leq \limsup{a_n}+\limsup{b_n}.$$
\item[b)] Dokažite da ako je $(a_n)$ uz to i konvergentan, da vrijedi
$$\limsup{(a_n+b_n)}=\lim\limits_{n\to\infty}{a_n}+\limsup{b_n}.$$
\end{itemize}
\end{exercise}
\begin{proof}[Rješenje]
a) Dokažimo prvo drugu nejednakost. Ideja će biti Kako je $\limsup{(a_n+b_n)}$ i sam gomilište niza $(a_n+b_n)$, postoji podniz $(a_{p_n}+b_{p_n})$ takav da je $\lim\limits_{n\to \infty}{\left(a_{p_n}+b_{p_n}\right)}=\limsup{(a_n+b_n)}$. Uočimo da je $\limsup{a_n}\geq \limsup{a_{p_n}}$ i $\limsup{b_n}\geq \limsup{b_{p_n}}$ (u suprotnome bi imali kontradikciju s definicijom limesa superiora). No niz $\left(a_{p_n}\right)$ ima podniz $\left(a_{p_{q_n}}\right)$ takav da je $\lim\limits_{n\to \infty}{a_{p_{q_n}}}=\limsup{a_{p_n}}$. Sada iz konvergencije niza $\left(a_{p_{q_n}}+b_{p_{q_n}}\right)$ i konvergencije niza $\left(a_{p_{q_n}}\right)$ slijedi konvergencija niza $\left(a_{p_{q_n}}+b_{p_{q_n}}-a_{p_{q_n}}\right)=\left(b_{p_{q_n}}\right)$ k $\limsup{(a_n+b_n)}-\liminf{a_{p_n}}$. Imamo
\begin{align*}
\limsup{a_n}+\limsup{b_n}&\geq \limsup{a_{p_n}}+\limsup{b_{p_n}}=\lim\limits_{n\to \infty}{a_{p_{q_n}}}+\limsup{b_{p_n}}\\
&\geq \lim\limits_{n\to \infty}{a_{p_{q_n}}}+\lim\limits_{n\to \infty}{b_{p_{q_n}}}=\lim\limits_{n\to \infty}{\left(a_{p_{q_n}}+b_{p_{q_n}}\right)}=\limsup{(a_n+b_n)}.
\end{align*}
Dokažimo sada prvu nejednakost, i to koristeći već dokazanu drugu nejednakost. Uočimo da je $\limsup{\left(-a_n\right)}=-\liminf{a_n}$. Zaista, lako se vidi da ako s $S(x_n)$ označimo skup svih gomilišta niza $(x_n)$, vrijedi $S(-x_n)=-S(x_n)$, pa tvrdnja slijedi iz te činjenice i svojstava supremuma i infimuma. Vrijedi
\begin{align*}
\limsup{(a_n+b_n)}-\liminf{a_n}&=\limsup{(a_n+b_n)}+\limsup{\left(-a_n\right)}\geq \limsup{(a_n+b_n-a_n)}\\
&=\limsup{b_n},
\end{align*}
odakle direktno slijedi prva nejednakost.

b) Ako je $(a_n)$ konvergentan, onda je $\liminf{a_n}=\limsup{a_n}$, pa tvrdnja slijedi direktno iz a).
\end{proof}
U idućim zadatcima ćemo dati primjer jednog niza čiji je skup gomilišta jednak $[0, 1]$. Za $x\in \mathbb{R}$ uvedimo oznaku $\{x\}:=x-\lfloor x\rfloor$. Niz $(a_n)$ koji ćemo mi promatrati je zadan sa $a_n=\{\alpha n\}$, gdje je $\alpha\in \mathbb{I}$. Zapravo promatramo "decimalni dio" svakog broja $\alpha n$. Promotrimo npr niz $(a_n)$ zadan sa $a_n=\left\{\sqrt{2}n\right\}$, koji je prikazan na slici \ref{fig:6.2}.
\begin{figure}[ht]
\begin{center}
\begin{tikzpicture}
\begin{axis}[axis lines=middle,xlabel=\(x\),ylabel=\(y\),xmin=-0.1,xmax=15.1,ymin=-0.1,ymax=1.1, width=17cm, height=8cm]
...
\end{axis}
\end{tikzpicture}
\caption{Niz \((a_n)\) zadan s $a_n=\left\{\sqrt{2}n\right\}$}
\label{fig:6.2}
\end{center}
\end{figure}

Uočimo da je razlika između npr. $a_3$ i $a_8$, $a_{13}-a_8$, $a_{18}-a_{13}$ itd. u pravilu relativno mala. To vrijedi i općenito za članove ovog niza koji se razlikuju za $8-3=5$. Stoga, želimo pronaći podniz ovog niza čiji se svi članovi razlikuju za $5$, čime ćemo "usitniti" razmake između susjednih članova. Lako se vidi da je taj podniz npr. $(a_{5n})$. Ne samo to, nego je $a_8-a_3=a_5$, odakle vidimo da je $a_5$ blizak $0$. No treba paziti jer ovo ne vrijedi npr. za $a_2$ i $a_9$ (u tom redoslijedu), jer je $a_2-a_9=1-a_7$.

Općenito, ako je $k<l$ i $a_k<a_l$, onda je $a_l-a_k=a_{l-k}$, a ako je $a_k>a_l$ (pokazat ćemo da su svi članovi niza međusobno različiti pa ne može vrijediti $a_k=a_l$), onda je $a_k-a_l=1-a_{l-k}$. Kao što ćemo vidjeti, ovo nije slučajno, već ovo slijedi direktno iz svojstava funkcija $\lfloor \cdot\rfloor$ i $\left\{\cdot\right\}$. Mi ćemo pokazati da je moguće pronaći "po volji mali" element niza $(a_n)$ (odakle direktno slijedi da je $\liminf{a_n}=0$) pomoću čijih višekratnika ćemo se moći po volji blizu približiti bilo kojem broju iz skupa $[0, 1]$.
\begin{exercise}{Zadatak}
\label{Dirichletapprox}
Neka je $(a_n)$ niz zadan sa $a_n=\{\alpha n\}$, gdje je $\alpha\in \mathbb{I}$. Odredite $\liminf{a_n}$.
\end{exercise}
\begin{proof}[Rješenje]
Tvrdimo da je $\liminf{a_n}=0$. Neka je $\epsilon>0$. Uočimo da su svi članovi niza međusobno različiti. Zaista, kada bi postojali $k, l\in \mathbb{N}$, $k\neq l$ takvi da je $a_k=a_l$, vrijedilo bi $\{\alpha k\}=\{\alpha l\}$, tj. 
$$\alpha=\dfrac{\lfloor \alpha k\rfloor-\lfloor \alpha l\rfloor}{k-l}\notin \mathbb{Q}.$$
Uočimo da odavde i iz napomene \ref{bwlemma} slijedi da niz $(a_n)$ ima strogo monoton podniz (koji je ujedno i konvergentan), nazovimo ga $(a_{p_n})$. Tada je on i Cauchyjev, pa postoje $k, l\in \mathbb{N}$, $l<k$, takvi da je $0<\lvert\left\{\alpha p_k\right\}-\left\{\alpha p_l\right\}\rvert<\epsilon$.

Pretpostavimo da je $(a_{p_n})$ strogo rastući. Tada iz $a_{p_l}<a_{p_k}$ slijedi $\lfloor \alpha p_k\rfloor-\lfloor \alpha p_l\rfloor\leq \lfloor \alpha (p_k-p_l)\rfloor$ (v. napomenu \ref{floorrem}). Sada imamo
$$\left\{\alpha(p_k-p_l)\right\}=\alpha p_k-\alpha p_l-\lfloor \alpha(p_k-p_l)\rfloor= \alpha p_k-\alpha p_l-\left(\lfloor \alpha p_k\rfloor-\lfloor \alpha p_l\rfloor\right)=\left\{\alpha p_k\right\}-\left\{\alpha p_l\right\}<\epsilon.$$
Nadalje, kako niti jedan član niza $(a_n)$ očito nije jednak niti $0$ niti $1$ (u protivnom bi $\alpha$ bio iracionalan), vrijedi $\left\{\alpha(p_k-p_l)\right\}>0$ pa po zadatku \ref{limitpoints3} imamo da je $0$ gomilište niza $(a_n)$. 

Pretpostavimo da je $(a_{p_n})$ strogo padajući. Tada iz $a_{p_k}<a_{p_l}$ slijedi $\lfloor \alpha p_k\rfloor-\lfloor \alpha p_l\rfloor=1-\lfloor \alpha (p_k-p_l)\rfloor$, odakle direktno slijedi $\{\alpha p_l\}-\{\alpha p_k\}=1-\{\alpha (p_k-p_l)\}$ (v. napomenu \ref{floorrem}).
Imamo
$$0<1-\left\{\alpha(p_k-p_l)\right\}=\{\alpha p_l\}-\{\alpha p_k\}<\epsilon.$$
Dobivamo da je $1$ gomilište niza $(a_n)$, pa postoji podniz $(a_{q_n})$ koji konvergira u $1$. No taj podniz ima rastući podniz. Zaista, $a_{q_1}<1$, pa postoji $w\in \mathbb{N}$ takav da je $1-a_{q_w}<1-a_{q_1}$, tj. $a_{q_1}<a_{q_w}$. Sada ponavljanjem ovog postupka dolazimo do traženog podniza, nazovimo ga $\left(a_{q_{r_n}}\right)$. Sada dobivamo da je $0$ gomilište niza $(a_n)$ tako da primijenimo argument s prethodnog paragrafa na $\left(a_{q_{r_n}}\right)$ umjesto na $(a_{p_n})$.

Konačno, uočimo da je $0$ očigledno i najmanje gomilište niza jer bi u protivnom on sadržavao negativne elemente, što nije istina.
\end{proof}
\begin{remark}
\label{floorrem}
U rješenju prethodnog zadatka smo iskoristili činjenice da za sve $a, b\in \mathbb{R}$ takve da je $\{a\}<\{b\}$ vrijedi $\lfloor b\rfloor-\lfloor a\rfloor= \lfloor b-a\rfloor$ i $\lfloor a\rfloor-\lfloor b\rfloor= 1-\lfloor a-b\rfloor$. Dokažimo prvu tvrdnju, druga se dokazuje analogno. Zaista, vrijedi $a=\lfloor a\rfloor+\{a\}$ i $b=\lfloor b\rfloor+\{b\}$
pa imamo $b-a=\lfloor b\rfloor-\lfloor a\rfloor+\{b\}-\{a\}$. Uočimo da su $\{a\}, \{b\}\in [0, 1\rangle$, pa je $\{b\}-\{a\}<1$ i po pretpostavci $\{b\}-\{a\}>0$. Stoga je
$$\lfloor b\rfloor-\lfloor a\rfloor< b-a< \lfloor b\rfloor-\lfloor a\rfloor,$$
pa je $\lfloor b\rfloor-\lfloor a\rfloor= \lfloor b-a\rfloor$.
\end{remark}
\begin{exercise}{Zadatak}
Dokažite da je skup gomilišta niza $(a_n)$ iz zadatka \ref{Dirichletapprox} jednak $[0, 1]$.
\end{exercise}
\begin{proof}[Rješenje]
U dokazu ćemo se koristiti činjenicom da za $n\in \mathbb{N}$ i $x\in \mathbb{R}$ takve da je $n\{x\}<1$ vrijedi $n\{x\}=\{nx\}$, što će izravno slijediti iz činjenice da je u tom slučaju $n\lfloor x\rfloor= \lfloor nx\rfloor$ (v. napomenu \ref{floorrem2}).

U rješenju zadatka \ref{Dirichletapprox} smo pokazali da je $0$ najmanje gomilište. Lako se vidi da nema gomilišta strogo većih od $1$ jer u protivnom bi postojali elementi niza $(a_n)$ strogo veći od $1$. Pokazat ćemo da je da je svaki $a\in \langle 0, 1\rangle$ gomilište niza $(a_n)$, pa će iz zadatka \ref{limsupisalimitpoint} slijediti i da je $1$ gomilište. 

Pretpostavimo da postoji $a\in \langle 0, 1\rangle$ koji nije gomilište niza $(a_n)$. Tada po zadatku \ref{limitpoints3} postoji $\epsilon>0$ takav da skup $\langle a-\epsilon, a+\epsilon\rangle\setminus\left\{a\right\}$ ne sadrži niti jedan član niza $(a_n)$. Kako ista tvrdnja vrijedi i za svaki $\epsilon'<\epsilon$, bez smanjenja općenitosti možemo uzeti da je $\epsilon<\min{\left\{a, 1-a\right\}}$. Uočimo i da je $a+\epsilon<a+1-a=1.$ 

Uzmimo $m\in \mathbb{N}$ takav da je $\dfrac{1}{m}<\epsilon$ i da za svaki $x\in \left\langle 0, \dfrac{1}{m}\right\rangle$ postoji $k\in \mathbb{N}$ takav da je $kx\in \left\langle a, 1\right\rangle$ (egzistencija takvog $m$ slijedi iz zadatka \ref{lemmaforalimitpointexercise}) i $a_n$ takav da je $0<\{a_n\}<\dfrac{1}{m}$ (čija egzistencija je dokazana u rješenju zadatka \ref{Dirichletapprox}). Promotrimo $n_0\in \mathbb{N}$ sa svojstvom da je on najveći prirodan broj takav da je $n_0\{a_n\}<a$. Uočimo da je $$n_0\{a_n\}=\{n_0 a_n\}\leq a-\epsilon,$$
jer u protivnom bi to bio član skupa $\langle a-\epsilon, a+\epsilon\rangle\setminus\left\{a\right\}$. Uočimo i da je po definiciji $(n_0+1)\{a_n\}\geq a$. Vrijedi i da je $(n_0+1)\{a_n\}<1$, jer u suprotnom ne bi postojao višekratnik od $\{a_n\}$ koji je element skupa $\left\langle a, 1\right\rangle$. Stoga je i 
$$(n_0+1)\{a_n\}=\{(n_0+1)a_n\}\geq a+\epsilon\;\;\vee\;\; (n_0+1)\{a_n\}=a.$$
Ako je $(n_0+1)\{a_n\}=a$, dobivamo
$$\{a_n\}=(n_0+1)\{a_n\}-n_0\{a_n\}\geq a-a+\epsilon=\epsilon,$$
što je kontradikcija s činjenicom da je $\{a_n\}<\dfrac{1}{m}<\epsilon$. Analogno dobivamo kontradikciju i u drugom slučaju.
\end{proof}
\begin{remark}
\label{floorrem2}
Dokažimo da za sve $n\in \mathbb{N}$ i $x\in \mathbb{R}$ takve da je $n\{x\}<1$ vrijedi $n\lfloor x\rfloor= \lfloor nx\rfloor$, čime smo se koristili u rješenju prethodnog zadatka. Uočimo da je po pretpostavci
$$n\{x\}=nx-n\lfloor x\rfloor<1\Rightarrow nx<1+n\lfloor x\rfloor,$$
pa kada bi bilo $\lfloor nx\rfloor>n\lfloor x\rfloor$, vrijedilo bi
$$1+n\lfloor x\rfloor\leq \lfloor nx\rfloor\leq nx<1+n\lfloor x\rfloor.$$
pa mora vrijediti $\lfloor nx\rfloor\leq n\lfloor x\rfloor$. Obratna nejednakost slijedi lagano iz činjenice da je $n\lfloor x\rfloor\leq nx$, pa je $n\lfloor x\rfloor\leq \lfloor nx\rfloor$.
\end{remark}
\newpage
...
\end{multicols}
\end{exercise}
\begin{exercise}{Zadatak} Ispitajte konvergenciju niza \((a_n)\) i ako je konvergentan odredite mu limes, ako je:
\begin{multicols}{2}
\begin{itemize}
\item[a)] \(a_n=\sqrt[n]{n2^n+1}\),
\item[b)] \(a_n=\dfrac{1}{n^2+1}+\dfrac{1}{n^2+2}+\dots+\dfrac{1}{n^2+n}\),
\item[c)] \(a_n=1+\dfrac{2}{(-1)^n+3^n}\),
\item[d)] \(a_n=\dfrac{\ln{n}}{n^2+1}\),
\item[e)] \(a_n=\dfrac{n!}{n^n}\),
\item[f)] \(a_n=\dfrac{\lfloor n+1\rfloor+1}{\lfloor 2n+3\rfloor}\).
\end{itemize}
\end{multicols}
\end{exercise}
\begin{exercise}{Zadatak} \textbf{}
\begin{itemize}
\item[a)] Ispitajte konvergenciju niza $(a_n)$, gdje je
$$a_n=\left(1-\dfrac{1}{2}\right)\left(1-\dfrac{1}{4}\right)\dots \left(1-\dfrac{1}{2(n-1)!}\right)\left(1-\dfrac{1}{2n!}\right).$$
\item[b)] Neka su \((a_n)\) i \((b_n)\) dva niza takva da je \(a_n\leq b_n\), \((a_n)\) raste i \((b_n)\) pada. Dokažite da tada oba niza konvergiraju. Jesu li njihovi limesi općenito jednaki?
\end{itemize}
\end{exercise}
\begin{exercise}{Zadatak} (Ovaj zadatak je malo neobičan...)
\begin{itemize}
\item[a)] Odredite sve polinome \(P : \mathbb{R}\to \mathbb{R}\) takve da je
\[\lim\limits_{n\to\infty}{\dfrac{P(n)}{n^2+2n+5}}=2025.\]
\item[b)] Koliko ima prirodnih brojeva \(n\in \mathbb{N}\) za koje vrijedi
\[\dfrac{2n}{n+1}-\dfrac{3}{2^n}-\dfrac{1}{n}-\dfrac{2}{n^4}>1?\]
Dokažite svoje tvrdnje! (\textbf{Uputa:} Iskoristite definiciju limesa niza.)
\item[c)] Dokažite da $x\mapsto 2^x$ nije polinom. (\textbf{Uputa:} Pretpostavite suprotno i promatrajte niz $n\mapsto \dfrac{P(n)}{2^n}$, gdje je $P$ polinom. Kako je ovo kvocijent dva polinoma, možemo li nešto zaključiti o njihovim stupnjevima?)
\end{itemize}
\end{exercise}
\begin{exercise}{Zadatak} \((**)\)
...
\begin{exercise}{Zadatak} Neka su \((a_n),\,(b_n)\, (c_n)\) nizovi zadani formulama 
...
\end{exercise}
\begin{exercise}{Zadatak} $(*)$
Neka je \((a_n)\) konvergentan niz takav da je \(a_n\neq 0\) za sve \(n\in \mathbb{N}\). Konvergira li općenito niz \(\left(\dfrac{a_{2n}}{a_n}\right)\)? Za one $(a_n)$ za koje on konvergira, što sve može biti njegov limes? (\textbf{Uputa:} Iskoristite tvrdnju zadatka \ref{setoflimits}.)
\end{exercise}
\subsection*{Limesi složenijih nizova. Cauchyjevi nizovi}
\begin{exercise}{Zadatak} \textbf{}
\begin{itemize}
...
\begin{exercise}{Zadatak} Ispitajte konvergenciju niza \((a_n)\) i ako je konvergentan odredite mu limes, ako je:
\begin{itemize}
\item[a)] \(a_n=\dfrac{1}{(2n)!^2}\cdot\sum\limits_{i=1}^{2n}{i!^2}\),
\item[b)] \(a_n=\dfrac{\sum\limits_{i=n}^{2n}{(-i)^i}}{(2n)^{2n}}\),
\end{itemize}
\end{exercise}
\begin{exercise}{Zadatak}
Dokažite da je niz $(a_n)$ zadan s $a_n=1+\dfrac{1}{2^2}+\dfrac{1}{3^2}+\dots+\dfrac{1}{n^2}$ konvergentan na način da pokažete da je Cauchyjev. $\left(\text{\textbf{Uputa:} Iskoristite } \dfrac{1}{n(n+1)}=\dfrac{1}{n}-\dfrac{1}{n+1}.\right)$
\end{exercise}
\begin{exercise}{Zadatak} $(*)$
Dokažite da je niz realnih brojeva Cauchyjev ako i samo ako je konvergentan.
\end{exercise}
\begin{exercise}{Zadatak} \((**)\)
...
\begin{exercise}{Zadatak}
Neka je \((a_n)\) niz zadan formulom \(a_n=(-1)^n\left(1-\dfrac{1}{3^n}\right)\). Odredite skup svih gomilišta niza $(a_n)$. (Dokažite sve svoje tvrdnje.)
\end{exercise}
\begin{exercise}{Zadatak} \textbf{}
\begin{itemize}
\item[a)] Neka su $(a_n)$ i $(b_n)$ ograničeni nizovi takvi da su $a_n, b_n\geq 0$ za sve $n\in \mathbb{N}$. Dokažite da vrijedi
$$\liminf{a_n}\liminf{b_n}\leq \liminf{a_nb_n}\leq \liminf{a_n}\limsup{b_n}.$$
\item[b)] Dokažite da ako je $(b_n)$ konvergentan, da vrijedi
$$\liminf{a_nb_n}=\liminf{a_n}\cdot\lim\limits_{n\to \infty}{b_n}.$$
\end{itemize}
\end{exercise}
\begin{exercise}{Zadatak}
Neka je $(a_n)$ ograničen i $(b_n)$ niz sa svojstvom da za svaki $m\in \mathbb{N}$ postoji $n_0\in \mathbb{N}$ takav da za svaki $n\in \mathbb{N}$ takav da je $n\geq n_0$ vrijedi $b_n\in \{a_{m}, a_{m+1}, a_{m+2}, \dots\}$. Dokažite da je skup gomilišta od $(b_n)$ sadržan u skupu gomilišta od $(a_n)$.
\end{exercise}
\begin{exercise}{Zadatak} \((*)\)
Neka su \(p, q\in \mathbb{N}\), \(1<p<q\). Dokažite da svaki niz s točno \(q\) gomilišta ima podniz s točno \(p\) gomilišta.
\end{exercise}
\begin{exercise}{Zadatak} \((*)\)
Dokažite da svaki konvergentan niz postiže ili svoj infimum, ili svoj supremum, ili oboje. Dajte primjer sva tri tipa niza.
\end{exercise}
\begin{exercise}{Zadatak} \((**)\)
Kroz ovaj zadatak dat ćemo smjernice za dokaz Osnovnog teorema algebre -- klasičnog teorema koji kaže da ako su $a_0, a_1, \dots, a_n$ kompleksni brojevi, $a_n\neq 0$ i $P : \mathbb{C}\to \mathbb{C}$,
$$P(z)=a_nz^n+a_{n-1}z^{n-1}+\dots+ a_1z+a_0,$$
tada za svaki polinom koji nije konstanta postoji barem jedan $z_0\in \mathbb{C}$ takav da je $P(z_0)=0$, tj. svaki polinom u $\mathbb{C}$ koji nije konstanta ima nultočku.
\begin{itemize}
\item[a)] Promotrite skup $B=\left\{ \left\lvert P(z)\right\rvert : z\in \mathbb{C}\right\}$, te dokažite da je $\inf{B}\in B$. To možete napraviti tako da pretpostavite da je $\inf{B}\notin B$, te tada imamo da postoji niz $(b_n)$ u $B$ koji konvergira u $\inf{B}$. Koristeći niz $(b_n)$, konstruirajte niz $(a_n)$ koji konvergira u $\inf{B}$ i čiji je limes element skupa $B$.
\item[b)] Kako je $\inf{B}\in B$, postoji $z_0\in \mathbb{C}$ takav da je $\inf{B}=\left\lvert P(z_0)\right\rvert$. Tvrdimo da je $P(z_0)=0$. U suprotnome, kad bi bilo $P(z_0)\neq 0$, dođite do kontradikcije tako što ćete dokazati da za svaki $a\in \mathbb{C}$, $a\neq 0$, postoji $h\in \mathbb{C}$ takav da je $\left\lvert P(a+h)\right\rvert<\left\lvert P(a)\right\rvert$. To možete dokazati tako da primijetite da je
$$P(a+h)=P(a)+Ah^m+Ah^m\left(A_1h+\dots+ A_{n-m}h^{n-m}\right),$$
gdje je $n$ stupanj od $P$, $m$ stupanj od $h$, te $A, A_1, A_2, \dots A_{n-m}$ neki kompleksni brojevi i $A\neq 0$. Ovo pak možete pokazati tako da iskoristite činjenicu da postoji $h_0\in \mathbb{C}$ takav da je
$$h_0^m=-\dfrac{1}{2}\dfrac{P(a)}{A}$$
i da se izraz $\left(A_1h+\dots+ A_{n-m}h^{n-m}\right)$ može napraviti "po volji malim po modulu".
\end{itemize}
(Rješenje ovog zadatka možete vidjeti u \cite{14}, u Dodatku.)
\end{exercise}