...
\item[b)] \(f(x)=\left\lvert \left\lvert x\right\rvert-4\right\rvert\).
\end{itemize}
\end{exercise}
...
\item Graf funkcije \(x\mapsto \left\lvert \left\lvert x\right\rvert-4\right\rvert\) je dobiven zrcaljenjem cijelog dijela grafa funkcije \(x\mapsto |x|-4\) ispod osi \(x\) u odnosu na os \(x\), a ostali dijelovi su nepromijenjeni. Njezin graf je na slici \ref{gr1} prikazan punom linijom. \qedhere
\end{itemize}
\end{proof}
\begin{figure}[ht]
...
\end{axis}
\end{tikzpicture}
\caption*{Graf funkcije \(x\mapsto \left\lvert \left\lvert x\right\rvert-4\right\rvert\)}
\end{subfigure}
...
\item[b)] \(f(x)=e^{|x|}\).
\item[c)] \(f(x)=x-\lfloor x\rfloor\) (\textbf{Uputa:} $\lfloor x\rfloor$ označava najveći cijeli broj manji ili jednak $x$.)
\end{itemize}
\end{exercise}
...
\end{figure}
\end{proof}
\begin{exercise}{Zadatak} Neka je \(f : \mathbb{R}\to \mathbb{R}\), \(f(x)=\sin{x}\). Graf funkcije \(f\) je zrcaljen po osi \(y\), zatim je pomaknut za dvije jedinične dužine udesno, a potom je skaliran u odnosu na ishodište, te je koeficijent tog skaliranja (homotetije) \(\dfrac{1}{2}\). Graf koje funkcije je novodobiveni skup?
\end{exercise}
...
\end{itemize}
\end{exercise}
\begin{exercise}{Zadatak} Promotrite sljedeću sliku.
\begin{figure}[ht]
\begin{center}
\begin{tikzpicture}
...
Ako je poznato da postoje \(a, b, c\in \mathbb{R}\), \(a\neq 0\) takvi da je graf na slici upravo graf funkcije \(f(x)=\left\lvert\left\lvert ax^2+bx+c\right\rvert-2\right\rvert\), odredite neke takve \(a, b, c\).
\end{exercise}
...
\item[c)] \(f : \mathbb{R}\to [-2, \infty\rangle\), \(f(x)=e^x-2\),
\end{itemize}
\end{exercise}
\newpage
\begin{exercise}{Zadatak} \textbf{}
...
\end{itemize}
\end{exercise}
\begin{exercise}{Zadatak} \((*)\) Neka je \(g : \mathbb{R}\to \mathbb{R}\) funkcija takva da je za svaku injekciju \(f : \mathbb{R}\to \mathbb{R}\), funkcija \(f+g\) također injekcija. Dokažite da je tada \(g\) konstanta.
\end{exercise}
...
\item[b)] Neka je \(f : \mathbb{R}\to [-2, \infty\rangle\) zadana formulom \(f(x)=\left\lvert \left\lvert \left\lvert x\right\rvert-2\right\rvert-2\right\rvert-2\). 
...
Odredite \(\mathcal{R}(f)\).
\end{exercise}
\begin{exercise}{Zadatak} \((*)\) Izračunajte
...
\end{multicols}
\end{exercise}
\begin{exercise}{Zadatak} Navedite primjer elementarne funkcije čija je prirodna domena:
\begin{multicols}{2}
\begin{itemize}
